<template>
  <div class="overflow-hidden relative bg-white pt-20 pb-12 mb-4 lg:pt-24 lg:pb-28 lg:mb-12 2xl:pt-40 2xl:pb-28">
    <div class="flex w-[90vw] mx-auto items-center mb-8 2xl:mb-12">
      <button type="button"
              @click="rotateBoard(-1)"
              class="p-2 md:text-2xl hidden lg:flex items-center justify-center"
              title="Tourner le plateau">
        <vue-feather type="rotate-ccw" stroke-width="1" size="2em"></vue-feather>
      </button>
      <h2 class="font-avante-titul-inline mx-auto text-center text-2xl md:text-4xl lg:text-6xl 2xl:text-7xl">
        Mon parcours
      </h2>
      <button type="button"
              @click="rotateBoard(1)"
    ...
    <div class="overflow-x-auto p-2">
      <div :style="[`transform: rotate(${rotation * 90}deg)`, `width: ${data.length * 150}px`]"
           class="font-josefin mx-auto lg:!w-[90vw] lg:h-[90vw] lg:text-[0.85vw] flex gap-1 lg:gap-0 justify-end flex-row-reverse lg:grid lg:grid-cols-board lg:grid-rows-board relative transition-transform duration-500">
        <BoardCase v-for="(caseData, index) in data"
                   ref="boardCases"
                   :data="caseData" class="board-case bg-white w-[150px] lg:w-auto"
        ...
        <!-- CENTER SQUARE START -->
        <div
            class="board-case-center hidden lg:block col-span-9 bg-transparent row-span-9 overflow-hidden w-full h-full transition-transform duration-500"
