...
const currentAnimation = ref<'left' | 'right' | MouseEvent | undefined>()

async function open(event: MouseEvent, from?: 'left' | 'right') {
  currentAnimation.value = from ?? event
...
function cardEnter(el: Element, onComplete: () => void) {
  gsap.set(el, { clearProps: 'all' })

  let from: gsap.TweenVars

  if (currentAnimation.value === 'left') {
    from = { x: '-100%', opacity: 0 }
  } else if (currentAnimation.value === 'right') {
    from = { x: '100%', opacity: 0 }
  } else {
    from = {
      scale: 0,
      left: currentAnimation.value?.clientX ?? 0,
      top: currentAnimation.value?.clientY ?? 0,
      rotateX: -120,
      rotateY: 50,
    }
  }

  gsap.from(el, { ...from, onComplete })
}

function cardLeave(el: Element, onComplete: () => void) {
  gsap.set(el, { clearProps: 'all' })

  let to: gsap.TweenVars
  if (currentAnimation.value === 'left' || currentAnimation.value === 'right') {
    to = { x: currentAnimation.value === 'left' ? '-100%' : '100%', scale: 0.5 }
  } else {
    to = {
      scale: 0,
      left: '50%',
  ...
  gsap.to(el, { ...to, onComplete })
}

function bgEnter(el: Element, onComplete: () => void) {
  gsap.set(el, { clearProps: 'all' })
  gsap.to(el, {
    opacity: 1,
    duration: 0.2,
...
function bgLeave(el: Element, onComplete: () => void) {
  gsap.set(el, { clearProps: 'all' })
...
<template>
  <Teleport to="body">
    <Transition :css="false" @enter="bgEnter" @leave="bgLeave">
      <div
        v-if="isActive"
    ...
    <Transition :css="false" @enter="cardEnter" @leave="cardLeave">
      <div
        v-if="isActive"
